...
class PhoneColaboradorController {
  ...
  async index({ request, response, view }) {
    const phoneColab = await PhoneColaborattor.query().with('phone.modelo').with('collaborator').with('chip').with('mobileEmail').fetch();
    return phoneColab;
  }

  async indexByColaboratorPhones({ request, params, response, view }) {
    const celulares = await PhoneColaborattor.query().with('phone.modelo.marca').with('collaborator').with('chip').with('mobileEmail').where('collaborators_id', '=', params.colab).where('data_devolucao', null).fetch();
    return celulares;
  }

  async indexAllPhoneInUse({ request, params, response , view}){
    const celularesEmUso = await PhoneColaborattor.query().with('phone.modelo').with('collaborator').with('chip').with('mobileEmail').whereNull('data_devolucao').fetch();
    return celularesEmUso;
  }


  async indexAllPhoneReturned({ request, params, response, view}){
    const returnedPhones = await PhoneColaborattor.query().with('phone.modelo').with('collaborator').with('chip').with('mobileEmail').whereNotNull('data_devolucao').fetch();
  ...
  async store({ request, auth, response }) {
    ...
    const phoneColab = await PhoneColaborattor.create({ usuario_criacao: auth.user.username, ...data });
